...
$uc_config = get_instance()->config['uc'];
define('UC_CONNECT', $uc_config['UC_CONNECT']);
define('UC_DBHOST', $uc_config['UC_DBHOST']);
define('UC_DBUSER', $uc_config['UC_DBUSER']);
define('UC_DBPW', $uc_config['UC_DBPW']);
define('UC_DBNAME', $uc_config['UC_DBNAME']);
define('UC_DBCHARSET', $uc_config['UC_DBCHARSET']);
define('UC_DBTABLEPRE', $uc_config['UC_DBTABLEPRE']);
define('UC_DBCONNECT', $uc_config['UC_DBCONNECT']);
define('UC_KEY', $uc_config['UC_KEY']);
define('UC_API', $uc_config['UC_API']);
define('UC_CHARSET', $uc_config['UC_CHARSET']);
define('UC_IP', $uc_config['UC_IP']);
define('UC_APPID', $uc_config['UC_APPID']);
define('UC_PPP', $uc_config['UC_PPP']);
...
function uc_api_input($data) {
	$s = urlencode(uc_authcode($data.'&agent='.md5(HTTP_USER_AGENT)."&time=".time(), 'ENCODE', UC_KEY));
	return $s;
}
...
function uc_avatar($uid, $type = 'virtual', $returnhtml = 1) {
	...
	$uc_avatarflash = UC_API.'/images/camera.swf?inajax=1&appid='.UC_APPID.'&input='.$uc_input.'&agent='.md5(HTTP_USER_AGENT).'&ucapi='.urlencode(str_replace('http://', '', UC_API)).'&avatartype='.$type.'&uploadSize=2048';
	if($returnhtml) {
