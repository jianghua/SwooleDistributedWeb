...
class BaseController extends SController
{
     ...
     */
    protected function getForm($modelObj, $form_name, $data=[])
    {
     ...
     */
    protected function checkForm($modelObj, $form_name, $input, &$error, &$data_arr=[])
    {
        if($modelObj->_form_secret)
        {
            if (method_exists($modelObj, 'getOwn')) {
     ...
     *
     * @author weihan
     * @datetime 2017年9月28日下午4:39:12
     */
    protected function getHostRoot()
    {
        $config = get_instance()->config;
        list($host) = explode(':', $this->request->header['host']??'');
        $root_path = $config['http']['root'][$host]['root']??'';
        if (empty($root_path)) {
            $root_path = $config['http']['root']['default']['root']??'';
        }
        if (!empty($root_path)) {
